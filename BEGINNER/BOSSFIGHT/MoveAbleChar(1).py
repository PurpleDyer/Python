# MoveAbleChar(1)
# A Character That Can Move In A Custom-Built Map In The Shape Of A Square
...
class Char:
    ...
    def move(self, direction):
        ...
        if keyboard.is_pressed(direction):
            if (direction == 'd' or direction == 'right') and (0 <= self.__x+1 < board.width):
                board.get_cell(self.__x, self.__y).clear_cell()
                self.__x += 1
                board.get_cell(self.__x, self.__y).add_content(self.__character)
            elif (direction == 'a' or direction == 'left') and (0 <= self.__x-1 < board.width):
                board.get_cell(self.__x, self.__y).clear_cell()
                self.__x -= 1
                board.get_cell(self.__x, self.__y).add_content(self.__character)
            elif (direction == 'w' or direction == 'up') and (0 <= self.__y-1 < board.height):
                board.get_cell(self.__x, self.__y).clear_cell()
                self.__y -= 1
                board.get_cell(self.__x, self.__y).add_content(self.__character)
            elif (direction == 's' or direction == 'down') and (0 <= self.__y+1 < board.height):
                board.get_cell(self.__x, self.__y).clear_cell()
                self.__y += 1
                board.get_cell(self.__x, self.__y).add_content(self.__character)
