# MoveAbleChar(3)
# A Character That Can Move In A Custom-Built Map In The Shape Of A Square But With Random Generated Blocks And Fruits Which You Can Eat By Going In Them
...
class Engine:
    def __init__(self, map_width, map_height, total_blocks, total_fruits):
        self.__map_width = map_width
        self.__map_height = map_height
        self.__total_blocks = total_blocks
        self.__total_fruits = total_fruits
    ...
    def run(self):
        ...
        fruits_cnt = 0
        while fruits_cnt < self.__total_fruits:
            cx = randint(0, self.__map_width-1)
            cy = randint(0, self.__map_height-1)
            if not board.get_cell(cx, cy).has_typ(object):
                board.get_cell(cx, cy).add_content(Fruit())
                fruits_cnt += 1
