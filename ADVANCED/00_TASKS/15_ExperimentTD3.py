...
class Engine:
    ...
    def run(self):
        ...
        board.get_cell(enemy2.x, enemy2.y).add_content(enemy2)

        def one_second_movers():
            while True:
                time.sleep(1)
                enemy.move()

        def half_second_movers():
            while True:
                time.sleep(0.5)
                enemy2.move()
        ...
        def printer():
            while True:
                print(board)
                time.sleep(0.0049876)
            
        one_second_movers_thread = threading.Thread(target=one_second_movers)
        half_second_movers_thread = threading.Thread(target=half_second_movers)
        printer_thread = threading.Thread(target=printer)

        one_second_movers_thread.start()
        half_second_movers_thread.start()
...
# results: COMPLETE
# P.O.D: making multiple enemies move on their own
# recommendation: MAKE MULTI-DIRECTIONAL MOVER