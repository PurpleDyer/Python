...
class Enemy:
    ...
    def move(self):
        ...
        if (0 <= self.__x+1 < len(main_board[0])) and (main_board[self.__y][self.__x+1].cget('text') == ' ') and (self.__damage > 0):
            while (0 <= self.__x+1 < len(main_board[0])) and (main_board[self.__y][self.__x+1].cget('text') == ' ') and (self.__damage > 0):
                time.sleep(1)
        ...
        elif (0 <= self.__y+1 < len(main_board)) and (main_board[self.__y+1][self.__x].cget('text') == ' ') and (self.__damage > 0):
            while (0 <= self.__y+1 < len(main_board)) and (main_board[self.__y+1][self.__x].cget('text') == ' ') and (self.__damage > 0):
                time.sleep(1)
                main_board[self.__y][self.__x].configure(text=' ')
                main_board[self.__y][self.__x].remove_content(self)
                self.__y += 1
...
def throw(x, y, master, text):
    ...
    if board[y][x] == 'O':
        button = TPath(master=master, width=10, height=5, background="white", font=Font_style)
        button.configure(command=partial_function(button, x, y))
        button.grid(column=x, row=y)
        return button

    elif board[y][x] == ' ':
        label = Path(master=master, text=text, width=10, height=5, background="red", font=Font_style)
...
damager_thread = threading.Thread(target=damager)
...
# problem: all characters dont move at the same time
# reason: while loops in Enemy.move()
