...
class AiEnemy:
    ...
    def move(self):
        ...
        if 0 <= self.__x+1 < board.width:
            ...
            while 0 <= self.__y-1 < board.height:
                time.sleep(0.5)
                board.get_cell(self.__x, self.__y).remove_content(self)
                self.__y -= 1
                board.get_cell(self.__x, self.__y).add_content(self)
...
# results: COMPLETE
# P.O.D: making an enemy go on its own in multiple directions (excluding going back)
# recommendation: MAKE AN ENEMY FIND ITS WAY FORWARD